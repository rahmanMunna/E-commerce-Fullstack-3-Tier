                        ...
                        <div className="overflow-x-auto bg-white shadow-lg rounded-xl border border-gray-100">
                            <table className="min-w-full divide-y divide-gray-200">
                                ...
                                <tbody className="divide-y divide-gray-200">
                                    {cancelledOrders.map((cancelOrder, idx) => (
                                        <tr
                                            key={idx}
                                        ...
                                        >
                                            ...
                                            <td className="px-6 py-4 text-gray-700">
                                                {convertDateFormate(cancelOrder?.Date)}
                                            </td>
                                            <td className="px-6 py-4">
                                                <span
                                                    className={`inline-block px-3 py-1 rounded-full text-xs font-semibold ${cancelOrder?.OrderStatus?.Status === "Delivered"
                                                        ? "bg-green-100 text-green-700"
                                                        : cancelOrder?.OrderStatus?.Status === "Pending"
                                            ...
                                            <td className="px-6 py-4 text-gray-700">
                                                {cancelOrder?.DeliveryManId !== 0 ? (
                                                    <span className="font-medium text-gray-800">
                                                        #{cancelOrder?.DeliveryManId}
                                                    </span>
                                                ) : (
                                            ...
                                            <td className="px-6 py-4 font-semibold text-blue-600">
                                                TK {cancelOrder?.Total}
                                            </td>
                                            <td className="px-6 py-4 flex justify-center gap-3">
                                                <Link
                                                    to={`/customerDashboard/trackOrderDetails/${cancelOrder.Id}`}
